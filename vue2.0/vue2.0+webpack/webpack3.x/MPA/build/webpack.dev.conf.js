...
// const HtmlWebpackPlugin = require('html-webpack-plugin-for-multihtml') 加快热更新使用这个，生产的时候不能使用（未深入理解）
...
const devWebpackConfig = merge(baseWebpackConfig, {
  ...
  devServer: {
    ...
    // 启用webpack的热替换特性
    hot: true,
    // 表示的是告诉服务器从哪里提供内容。（只有想提供静态文件时才需要）
  ...
  plugins: [
    ...
    new webpack.DefinePlugin({
      'process.env': Object.assign(require('../config/dev.env'), {
        ...
        // 'WX_APPID': JSON.stringify(process.env.WX_APPID)
      })
    }),
    // 启用热替换模块
    ...
    if (err) {
      ...
      devWebpackConfig.plugins.push(new FriendlyErrorsPlugin({
        ...
        compilationSuccessInfo: {
          messages: [`Your application is running here: http://${devWebpackConfig.devServer.host}:${port}${config.dev.assetsPublicPath}modules/***.html`]
        },
...
for (var pathname in pages) {
  // 配置生成的html文件，定义路径等
  let flag = config.dev.ignoreModeles.includes(pathname)
  if (flag) {
    continue
  }
  console.log('运行中的模块:', pathname);
  var conf = {
    ...
    // https://www.wandouip.com/t5i354046/
